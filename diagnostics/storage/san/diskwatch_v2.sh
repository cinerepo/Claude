...
while true; do
    ...
    clear
    printf "${CYAN}${BOLD}%-20s %10s %10s %10s %8s %10s %10s${RESET}\n" \
        "DEVICE" "READ MB/s" "WRITE MB/s" "AWAIT ms" "%UTIL" "R-INFLIGHT" "W-INFLIGHT"
    printf "${CYAN}%-20s %10s %10s %10s %8s %10s %10s${RESET}\n" \
        "--------------------" "----------" "----------" "----------" "--------" "----------" "----------"
    ...
    for dev in $(echo "${!DS_READ[@]}" | tr ' ' '\n' | sort); do
        ...
        # read /sys/block/<dev>/inflight for in-flight read/write counts
        inflight_file="/sys/block/$dev/inflight"
        if [[ -r "$inflight_file" ]]; then
            read -r r_inflight w_inflight < "$inflight_file"
        else
            r_inflight="-"
            w_inflight="-"
        fi
        ...
        # color inflight if high (>32 is notable for HDDs)
        [[ "$r_inflight" =~ ^[0-9]+$ ]] && ri_int=$r_inflight || ri_int=0
        [[ "$w_inflight" =~ ^[0-9]+$ ]] && wi_int=$w_inflight || wi_int=0
        if   (( ri_int >= 64 )); then rc=$RED
        elif (( ri_int >= 32 )); then rc=$YELLOW
        else rc=$GREEN
        fi
        if   (( wi_int >= 64 )); then wc=$RED
        elif (( wi_int >= 32 )); then wc=$YELLOW
        else wc=$GREEN
        fi

        printf "%-20s %10s %10s ${ac}%10s${RESET} ${uc}%8s%%${RESET} ${rc}%10s${RESET} ${wc}%10s${RESET}\n" \
            "$dev" "$read_mb" "$write_mb" "$await" "$util" "$r_inflight" "$w_inflight"
